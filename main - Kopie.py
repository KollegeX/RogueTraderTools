...
class MyFrame1(wx.Frame):
    def __init__(self, parent):
        ...
        gbSizer1.Add(
            self.btn_start_nav, wx.GBPosition(6, 0), wx.GBSpan(1, 1), wx.ALL, 5
        )
    ...
    def onSave(self, e):
        dlg = wx.FileDialog(
            self, "Save to file:", ".", "", "Navigator (*.Nav)|*.Nav", wx.FD_SAVE
        )
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if i == 0:  # Text format
                try:
                    ...
                    f.append(int(self.vi_nav.GetValue()))
                    f.append(int(self.vi_psy.GetValue()))
                    f.append(int(self.vi_nav_mod.GetValue()))
                    f.append(int(self.vi_psy_mod.GetValue()))
                    f.append(bool(self.m_checkBox7.GetValue()))
                    f.append(bool(self.m_checkBox8.GetValue()))
                    f.append(bool(self.m_checkBox9.GetValue()))
                    f.append(bool(self.m_checkBox10.GetValue()))
                    f.close()
                    self.m_textCtrl5.ChangeValue("Saved Setting")
                except:
                    self.m_textCtrl5.ChangeValue("Something went wrong!")

    def onLoad(self, e):
        self.dirname = ""
        dlg = wx.FileDialog(
            self, "Choose a file", self.dirname, "", "*.Nav*", wx.FD_OPEN
        )
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def start_flight(self, event):
        # event tuple acces tup1 = (9,8) tuple with tup1[0] for 9
